...
import { seedConjugationDrills } from './conjugation-seed';
import { tx } from '@/db';
...
export function seed(db: DB): {
  ...
  drills: number;
  vocab: number;
  /** Conjugation drills generated by the engine (see conjugation-seed.ts). */
  engineDrills: number;
} {
  return tx(db, () => {
    ...
    // Generated conjugation drills, which is what stops the 25 verb topics
    // from being empty until an API key exists.
    const engine = seedConjugationDrills(db);
